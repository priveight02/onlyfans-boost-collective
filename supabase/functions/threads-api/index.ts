...
async function threadsFetch(endpoint: string, token: string, method = "GET", body?: any) {
  ...
  if (!contentType.includes("application/json")) {
    ...
    if (resp.status === 404) {
      throw new Error("This Threads API endpoint is not available (404). This feature may require approved scopes.");
    }
  ...
  try {
    ...
    switch (action) {
      ...
      case "get_threads": {
        ...
        result = await threadsFetch(`/me/threads?fields=id,media_product_type,media_type,media_url,permalink,owner,username,text,timestamp,shortcode,thumbnail_url,children,is_quote_post&limit=${limit}${cursor}`, conn.access_token);
        break;
      }
      case "get_thread": {
      ...
      case "create_text_thread": {
        ...
        if (params.reply_control) containerParams.set("reply_control", params.reply_control);
        if (params.link_attachment) containerParams.set("link_attachment", params.link_attachment);
        ...
        if (result.id) {
          await supabase.from("social_posts").insert({
            account_id: account_id, platform: "threads", platform_post_id: result.id,
      ...
      case "create_carousel_thread": {
        ...
        if (params.reply_control) carouselParams.set("reply_control", params.reply_control);
      ...
      // ===== REPLIES =====
      case "get_replies": {
        result = await threadsFetch(`/${params.thread_id}/replies?fields=id,text,username,permalink,timestamp,media_type,media_url,shortcode,thumbnail_url,is_quote_post,hide_status&limit=${params?.limit || 25}`, conn.access_token);
        break;
      }
      case "get_conversation": {
        result = await threadsFetch(`/${params.thread_id}/conversation?fields=id,text,username,permalink,timestamp,media_type,media_url,hide_status&limit=${params?.limit || 25}`, conn.access_token);
        break;
      }
      case "hide_reply": {
      ...
      case "get_mentions": {
        const userId = conn.platform_user_id || "me";
        result = await threadsFetch(`/${userId}/mentions?fields=id,text,username,permalink,timestamp,media_type,media_url&limit=${params?.limit || 25}`, conn.access_token);
        break;
      }

      // ===== KEYWORD SEARCH =====
      case "keyword_search": {
        result = await threadsFetch(`/threads/search?q=${encodeURIComponent(params.query)}&fields=id,text,username,permalink,timestamp,media_type&limit=${params?.limit || 25}`, conn.access_token);
        break;
      }
      ...
      case "search_locations": {
        result = await threadsFetch(`/threads/location_search?q=${encodeURIComponent(params.query)}&fields=id,name,address&limit=${params?.limit || 10}`, conn.access_token);
        break;
      }
