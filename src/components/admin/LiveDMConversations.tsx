...
// (24h Pause Countdown removed — no cooldown system)
...
const LiveDMConversations = ({ accountId, autoRespondActive, onToggleAutoRespond, onNavigateToSession, igSessionId, igSessionStatus, igPlatformUserId, platformUserId: propPlatformUserId, platform = "instagram" }: LiveDMConversationsProps) => {
  ...
  const [dailyStats, setDailyStats] = useState<{ sent: number; limit: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const loadDailyStats = async () => {
      const { data } = await supabase.from("auto_respond_state").select("daily_sent_count, daily_limit").eq("account_id", accountId).maybeSingle();
      if (data) setDailyStats({ sent: data.daily_sent_count || 0, limit: data.daily_limit || 500 });
    };
    loadDailyStats();
  ...
  // Background sync top 5 convos every 1.5s
  useEffect(() => {
  ...
  return (
              ...
              <div className="overflow-y-auto" style={{ maxHeight: "min(480px, 60vh)" }}>
                <div className="p-2 space-y-px">
                  {[
                    ...
                    { key: "uncensoredMode", label: "Uncensored Mode", desc: "Disable scripted guardrails and force direct, human, uncensored conversation replies", checked: uncensoredMode, set: setUncensoredMode, onMsg: "Uncensored Mode ON: direct human-style replies", offMsg: "Uncensored Mode OFF" },
                  ].map((item) => {
    ...
    <div className="flex bg-white/[0.02] rounded-b-lg border border-t-0 border-white/[0.06] overflow-hidden" style={{ height: `calc((100vh - 260px) * ${uiScale})`, minHeight: 500 }}>
      {/* LEFT PANEL - Conversation List */}
      <div className={`w-[340px] min-w-[340px] border-r border-white/[0.06] flex flex-col ${selectedConvo ? "hidden lg:flex" : "flex"}`}>
        {/* Header */}
