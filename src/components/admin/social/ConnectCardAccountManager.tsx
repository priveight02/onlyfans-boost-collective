...
import { Settings, Plus, RefreshCw, User, Wifi, WifiOff, Clock, Trash2 } from "lucide-react";
...
const ConnectCardAccountManager = ({
  ...
  const activeConns = connections.filter(c => c.platform === platform && c.is_connected);
  const connCount = activeConns.length;
  const canAddMore = connCount < maxAccounts;

  // Load all accounts (including historical disconnected ones)
  useEffect(() => {
    ...
    load();
  }, [showSettings, user?.id, platform, connections]);
  ...
  return (
    <>
      {/* + icon top-left: add more accounts */}
      {connCount >= 1 && canAddMore && (
        <button
          ...
          title={`Add another ${platform} account (${connCount}/${maxAccounts})`}
        >
          <Plus className="h-2.5 w-2.5 text-muted-foreground group-hover/add:text-foreground transition-colors" />
        </button>
      )}

      {/* Settings wheel bottom-right: account management (only when 2+ connected) */}
      {connCount >= 2 && (
            ...
            >
              ...
              {canAddMore && (
                <div className="px-3 py-2 border-t border-white/[0.06]">
                  ...
                  >
                    <Plus className="h-3 w-3" /> Add {platform} Account ({connCount}/{maxAccounts})
                  </Button>
                </div>
              )}
            </div>
