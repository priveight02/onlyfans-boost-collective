...
const ContentSandbox = ({ items, onRefresh }: { items: any[]; onRefresh: () => void }) => {
  ...
  return (
    <div ref={wrapperRef} data-sandbox-wrapper className="flex flex-col gap-1 w-full min-w-0 overflow-hidden pr-1" style={{ height: "calc(100vh - 64px)" }}>
      ...
      {sandboxListOpen && (
        <div onPointerDown={(e) => e.stopPropagation()} className="fixed z-[9999] rounded-xl bg-[hsl(222,35%,8%)] border border-white/[0.08] shadow-2xl backdrop-blur-xl p-2 w-[300px] max-h-[420px] overflow-hidden flex flex-col" style={{ bottom: "40px", left: "8px" }}>
